...
;; - Only accepted candidates can fulfill
...
(deftest-smart-contract-go candidates-test {}
  (let [hex  (partial web3-utils/to-hex @web3)
        ...
        ;; since candidate1 was accepted should be able to fulfill the job
        ff1-tx-receipt (<? (ethlance-jobs/fulfill-job ethlance-jobs-address
                                                      [candidate1
                                                       job-id
                                                       [candidate1]
                                                       jobs-data-hash
        ...
        ;; since candidate2 was NOT accepted it should NOT be able to fulfill the job
        ff2-tx-receipt (<? (ethlance-jobs/fulfill-job ethlance-jobs-address
                                                      [candidate2
                                                       job-id
                                                       [candidate2]
                                                       jobs-data-hash
    ...
    (testing "Only accepted candidates can fulfill a job"
      (let [ev (<! (ethlance-issuer/ethlance-jobs-event-in-tx :JobFulfilled ff1-tx-receipt))]
        (is (= (:_job-id ev) job-id))
        (is (= (js->clj (:_fulfillers ev)) [candidate1]))
        (is (= (:_ipfs-hash ev) jobs-data-hash))
        (is (= (:_submitter ev) candidate1))
        (is (= (:_amount ev) "10")))

      (is (nil? ff2-tx-receipt)))))
